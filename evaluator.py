...
from sklearn.metrics import accuracy_score, matthews_corrcoef, mean_squared_error, f1_score, classification_report, confusion_matrix
...
def evaluate(prediction, ground_truth, hinge=False, reg=False):
    ...
    performance = {}
    if reg:
        performance['mse'] = mean_squared_error(
            np.squeeze(ground_truth), np.squeeze(prediction))
        return performance
    ...
    performance['cr'] = confusion_matrix(ground_truth, pred)
    return performance
